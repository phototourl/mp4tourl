...
import { DOCUMENT_UPLOAD_ACCEPT, isClientDocumentUploadCandidate } from "@/lib/document-upload-allowlist";
...
function loginCallbackPath(locale: string): string {
  return locale === routing.defaultLocale ? "/file-to-url" : `/${locale}/file-to-url`;
}

export function FileToUrlTool() {
  const t = useTranslations("fileToUrl.tool");
  const tPage = useTranslations("fileToUrl.page");
  ...
  const pickFile = (f: File | null | undefined) => {
    ...
    if (!isClientDocumentUploadCandidate(f)) {
      setFile(null);
